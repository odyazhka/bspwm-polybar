...
fifo_path() {
    echo "${STATE_DIR}/fifo_${1//[^a-zA-Z0-9_-]/_}"
}
...
action_toggle() {
    local mon="$1" fifo cur
    fifo="$(fifo_path "$mon")"
    cur="$(get_mode "$mon")"
    [[ "$cur" == "collapsed" ]] && set_mode "$mon" "expanded" || set_mode "$mon" "collapsed"
    echo "toggle" > "$fifo"
}
...
run_loop() {
    ...
    while read -r _event; do
        render "$mon"
    done < <(bspc subscribe desktop_focus desktop_add desktop_remove desktop_rename node_transfer & tail -f "$fifo")
}
